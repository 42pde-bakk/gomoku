...
int Heuristic::get_h() const { return (this->h); }

uint8_t Heuristic::get_player() const { return (this->player); }

unsigned int Heuristic::get_length(unsigned int &idx, unsigned int stone_p,
                                   unsigned int d,
                                   bool &empty_space_inbetween) const {
...
LineValue Heuristic::calc_linevalue(unsigned int length,
...
unsigned int Heuristic::count_open_sides(unsigned int prev,
...
void Heuristic::tryUpgradeLineVal(LineValue &lv, unsigned int prev,
                                  unsigned int next, const int dir,
...
void Heuristic::count_lines(unsigned int start_idx, unsigned int stone_p) {
  ...
  for (unsigned int d = 0; d < dirs.size(); d++) {
    ...
    unsigned int length =
        this->get_length(next, stone_p, d, empty_space_inbetween);
...
void Heuristic::print_heuristic(std::ostream &o) const {
  static const LineValue linevalues[] = {LineValue::HALF_OPEN_TWO,
                                         LineValue::OPEN_TWO,
                                         LineValue::HALF_OPEN_THREE,
                                         LineValue::OPEN_THREE,
                                         LineValue::HALF_OPEN_FOUR,
                                         LineValue::OPEN_FOUR,
                                         LineValue::FIVE};
...
bool Heuristic::has_winner() const { return (this->winner != 0); }
...
uint8_t Heuristic::get_winner() const { return (this->winner); }

Heuristic::Heuristic() {}

Heuristic::Heuristic(const Heuristic &x)
    : Bitboard(x), values(), h(x.h), winner(x.winner), player(x.player),
      captures(x.captures) {
  values[0].fill(0);
  values[1].fill(0);
}
...
bool Heuristic::canGetCaptured(unsigned int start_idx, int dir) const {
  ...
  while (idx < REALBOARDSIZE && !isSeperatingBitIndex(idx) &&
         this->bitboard_get(idx) == stone_p) {
    ++length;
    idx += dir;
  }
  unsigned int back_idx = start_idx + dir_opp;
  while (back_idx < REALBOARDSIZE && !isSeperatingBitIndex(back_idx) &&
         this->bitboard_get(back_idx) == stone_p) {
    ++length;
    back_idx += dir_opp;
  }
  if (length != 2 || idx >= REALBOARDSIZE || isSeperatingBitIndex(idx) ||
      back_idx >= REALBOARDSIZE || isSeperatingBitIndex(back_idx))
    return (false);
  return (tile_is_empty(idx) ^ tile_is_empty(back_idx));
  //	return ((int)tile_is_empty(idx) + (int)tile_is_empty(back_idx) == 1);
}

bool Heuristic::isUnbreakable(unsigned int start_idx, unsigned int end_idx,
                              int dir) const {
  static const std::array<int, 4> dirs = setup_dirs();
...
bool Heuristic::enoughSpaceForFiveInARow(unsigned int idx, int dir, int opp_dir,
