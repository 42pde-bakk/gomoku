...
class Gamestate:
	...
	def __deepcopy__(self, memodict={}):
		out = Gamestate(self)
		out.board = np.ndarray.copy(self.board)
		out.captures = self.captures.copy()
		return out
	...
	def player_check(self, y: int, x: int, player_to_check: int) -> bool:
		return y < 0 or y >= 19 or x < 0 or x >= 19 or self.board[y][x] != player_to_check
	...
	def capture(self, pos1: tuple, pos2: tuple, capturing_player: Stone) -> None:
		...
		self.board[pos1_y][pos1_x] = Stone.EMPTY.value
		self.board[pos2_y][pos2_x] = Stone.EMPTY.value
	...
	def game_over_check(self, y: int, x: int, player: Stone) -> bool:
		for dy, dx in [(1, 0), (0, 1), (1, 1), (-1, 1)]:
			n = m = 1
			while self.board[y + n * dy][x + n * dx] == player.value:
				n += 1
			while self.board[y + m * -dy][x + m * -dx] == player.value:
				m += 1
			if n + m + 1 >= 5:
				# self.winner = player
	...
	def generate_children(self, player: int) -> list:
		...
		if self.winner or any(c >= 10 for c in self.captures):
			return []
		for (y, x), item in np.ndenumerate(self.board):
